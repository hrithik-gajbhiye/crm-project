...
export const updateTaskChecklist = async (req,res,next) =>{
    try {
        ...
        // 4 task, 4 complete completedCount / totalItems = 1 *100 = 100
...
export const getDashboardData = async (req,res,next) =>{
    ...
    const taskDistribution = taskStatuses.reduce((acc,status) =>{
        const formattedKey = status.replace(/\s+/g,"") // remove space for response key
    ...
    const recentTasks = await Task.find().sort({createdAt : -1}).limit(10).select("title status priority due date createdAt")
...
export const userDashboardData = async (req,res,next)=>{
    ...
    const recentTasks = await Task.find({assignedTo:userObjectId}).sort({createdAt:-1}).limit(10).select("title status priority due date createdAt")
